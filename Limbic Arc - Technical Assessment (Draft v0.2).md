...
**Status:** Draft v0.2 — for review and iteration
**Date:** 2026-05-31
...
> **Confidence & verification note.** Findings are tagged with confidence levels — **HIGH** (directly observed in screenshots/write-up or vendor-documented), **MEDIUM** (strongly implied and standard for this class of system), **LOW / ASSUMPTION** (inferred, needs confirmation). Items marked **[VERIFY]** must be validated before acting. This is a draft; nothing here is a final architectural decision.

### Changelog
- **v0.2** — Incorporated client answers to open questions (§13): removed double-charge framing (tokenization fragmentation retained as the core payment risk); fixed Profile source-of-truth to Exigo; added "Limbic Arc sites should hold **no** card data / be **out of PCI scope**" as a target principle; corrected the replicated-site characterization (low *functionality*, used by *all* users); added **"no in-house technology team"** as a first-class constraint; expanded the §12 roadmap.
- **v0.1** — Initial draft.
...
Limbic Arc runs a direct-selling (MLM) business on two primary platforms: **Exigo** as the MLM/commission engine and back office, and **Fluid (fluid.app)** as the modern e-commerce/shopping experience. Exigo is a strong commission and genealogy engine; Fluid is a strong storefront and rep-enablement app. The problem is not either platform individually — it is the **seam between them**, the legacy **Exigo .NET boilerplate** the web properties are built on, and the fact that **Limbic Arc has no in-house technology team** to own either.
...
5. **No in-house technology team.** Limbic Arc currently has no internal engineering capability, which is *why* it is locked into Exigo's queue and stuck in reactive "firefighting" mode. **This constraint shapes every recommendation below** — modernization must be sequenced around standing up (or contracting) delivery capability, not assumed.

**Headline recommendation:** Adopt a **Strangler Fig** modernization strategy — incrementally extract the bottleneck domains (Profile, Payments/Tokenization, Subscriptions, Family & Friends, Signup/Upgrade) out of the Exigo boilerplate into independently deployable, API-fronted services, with an explicit, enforced **source-of-truth contract** per domain. Keep Exigo as the system of record for *commissions, genealogy, and profile*, let Fluid own *commerce and tokenization*, and stop retrofitting Exigo for things it was never designed to do. **Because there is no in-house team, Phase 0/1 must first establish delivery capability** (build a small team and/or engage an integration partner) before extraction begins.
...
| 7 | **Data / Analytics** | Google Analytics + Power BI | Reporting | Split across sources |

**Confidence:** HIGH for systems 1–5 (directly evidenced). System 6 per write-up.

**On the "replicated site" [client-confirmed]:** Its *low usage* reflects **low functionality, not few users** — the boilerplate-derived flows are thin, yet **all users pass through them**. It therefore sits on the critical path and **cannot be dismissed or retired as "low priority."** Any change to login, profile, or subscription touches surfaces every user sees.
...
The complaints in the write-up are symptoms of a small number of root causes. Naming them keeps the recommendations honest.

**Root cause A — Exigo boilerplate was never meant to be a long-term product surface.**
Exigo's strength is the commission/genealogy engine and its data architecture (200+ APIs, replicated data, SOC 2 Type II). The .NET boilerplate "replicated site" code ships to *bootstrap* a customer onto Exigo's own commerce flows — it is a starter kit, not a product platform. Limbic Arc has retrofitted it to do things (custom subscriptions, third-party commerce, Family & Friends, Pet InfoBoost upsells) it was not designed for. *(Confidence: HIGH.)*

**Root cause B — No separation of concerns in the legacy front end.**
The customer and distributor sites have no front-end/back-end boundary; presentation, business logic, and Exigo data access are interleaved. Combined with thin documentation, **every change requires Exigo tribal knowledge**, so Limbic Arc cannot move without Exigo, and Exigo's priority queue sets Limbic Arc's release timeline. *(Confidence: HIGH.)*

**Root cause C — No designated source of truth per domain.**
Two platforms (Exigo, Fluid) each independently hold profile/subscription/payment data, so the same business object exists in two places with divergent state. This is the direct mechanism behind tokenization fragmentation and subscription/billing inconsistency. *(Confidence: HIGH.)*

**Root cause D — Missing platform engineering foundations.**
No RBAC, limited admin tooling, no CI/CD discipline, reactive observability. These don't cause the architecture problem but they *amplify* it: every fix is slow, risky, and hard to verify. *(Confidence: HIGH.)*

**Root cause E — No in-house technology team. [client-confirmed]**
Limbic Arc has no internal engineering capability. This is the upstream cause of B and D and the reason the company is locked into the Exigo queue: with no one to own the code, every change is outsourced to the platform vendor on the vendor's timeline. **No amount of architecture fixes this on its own** — capability must be built or contracted. *(Confidence: HIGH — client-confirmed.)*

> **Key reframing:** This is not "replace Exigo" and not "replace Fluid." It is **"own the seam and the surfaces" — and first, have someone to own them.** Extract the contested domains into Limbic-Arc-owned services with clear contracts to each platform, once delivery capability exists.

---

## 5. Deep Dive — Subscriptions & Payment Tokenization (the #1 technical risk)

The write-up correctly identifies **subscriptions as the main problem area**, with tokenization as the sharpest edge. This deserves its own treatment because it is where the split-source-of-truth problem does the most damage.

### 5.1 What's happening (the fragmentation)
...
- The result is divergent billing state across the two platforms and brittle, hard-to-reason-about subscription behavior. *(Confidence: HIGH for the fragmentation; exact downstream effects to confirm against billing records — [VERIFY].)*
...
2. **Limbic Arc's own sites should collect *no* card data and sit *out of PCI scope*.** *(Client intent — [VERIFY] current state.)* Card capture/update is delegated entirely to Fluid via **hosted fields / redirect / iframe**, so raw PAN never touches a Limbic Arc surface. This is both a risk reduction and a major **PCI-scope reduction** (potentially SAQ-A).
...
5. **Token-consistency reconciliation during migration.** While both systems still hold tokens, a scheduled reconciliation compares Exigo-side and Fluid-side billing records and **flags mismatches** so they're caught and corrected proactively rather than surfacing to customers. *(Defensive control for the transition window.)*

### 5.4 Reality check — the dual-tokenization period will persist

The write-up is right that dual tokenization "will continue for a long time." That makes the **reconciliation/flagging control (5.3.5) and the "no card data on Limbic Arc sites" principle (5.3.2) the near-term priorities** — they are cheaper and faster than full migration and shrink both risk and PCI scope immediately.
...
|--------|---------------|-----------|-----|--------------------------|
| **Profile / Identity** | Mixed (Exigo + Fluid) | **Exigo** *(client-confirmed)* | Identity must be singular; Exigo holds genealogy keyed to the person. A Profile *service* fronts it as an API, but Exigo remains the record. | Profile svc (facade over Exigo) → Fluid (push) |
...
**Profile decision [closed]:** Profile source of truth is **Exigo**. The recommended **Profile service** is an *API facade + event publisher* in front of Exigo-as-record — it does **not** introduce a competing identity store. This reconciles "extract profile into a separate application with APIs" (the write-up) with "Profile SoT = Exigo" (client direction): the *interface* is extracted; the *record* stays in Exigo.
...
### 7.3 Cross-cutting platform foundations

| Concern | Current | Recommendation | Confidence |
|--------|---------|----------------|-----------|
| **Delivery capability** | **None in-house** | Stand up a small product-engineering team and/or engage an integration partner **before** extraction; this is the gating dependency | HIGH |
| **Identity / SSO** | Re-login across systems | Central IdP (OIDC) so customer/affiliate session carries into the Web App and across surfaces — fixes the out-of-scope touchpoint | HIGH |
| **RBAC** | None on the sites | Role/claim model at the IdP + per-service authorization; admin vs. customer vs. affiliate vs. support | HIGH |
| **Hosting** | Shared/unclear | Dedicated cloud subscription/tenant for Limbic Arc (isolation, cost attribution, blast-radius control) | HIGH |
| **Observability** | Limited / reactive | Datadog — distributed tracing across the Exigo↔Fluid seam, token-consistency dashboards, synthetic checks on checkout/card-update | HIGH |
| **CI/CD** | Not defined | Branch strategy, automated build/test gates, IaC, progressive deploys; enforce test-first discipline as the team builds new services | HIGH |
| **Analytics** | GA + Power BI (capture only) | Define business-health metrics (§9) and a semantic layer; move from "we have dashboards" to "dashboards drive decisions" | MEDIUM |
...
- **Define the business-health metric set first**, then instrument. Candidates for a subscription MLM:
  ...
  - **Billing integrity:** failed-charge / dunning rate, **token-mismatch / reconciliation-exception count** (ties to §5).
...
The write-up's framing — *playing defense when we should be on offense* — is correct, and **the no-in-house-team constraint makes AI-assisted delivery especially relevant**: a small team augmented with AI development can punch above its weight. The architecture work is the prerequisite — AI features are hard on coupled boilerplate, easy on clean APIs + unified data.

- **Internal / developer velocity:** AI-assisted development to help a lean team move quickly and reduce firefighting; automated triage of support/billing issues.
...
- **Guardrail:** AI features should consume the *Profile/Subscription/Payments APIs*, never the legacy boilerplate directly — otherwise they inherit the coupling. *(Confidence: MEDIUM — opportunity framing.)*
...
- **Workstreams:**
  - Stand up a small product-engineering team **and/or** engage a vetted Exigo/Fluid integration partner; define the operating model (who owns what, on-call, change process).
  ...
  - Quick defensive wins: audit logging on profile/subscription/payment writes; token-consistency reconciliation/flagging (§5.3.5); freeze *new* Exigo-side tokenization wherever avoidable.
- **Key deliverables:** Team/partner in place; current-state integration map; reconciliation exception report; decision log.
- **Dependencies:** Budget + hiring/contracting decision (executive).
...
- **Workstreams:**
  ...
  - Confirm and reduce **PCI scope** to (near) zero on Limbic Arc surfaces *(targets §13.3/§13.4 verifications)*.
  - **Subscriptions service** owning lifecycle on Fluid; emit volume/commission events to Exigo (outbox + idempotent consumers).
  - Migration plan for subscriptions currently in Exigo → Fluid, sized by the §13 volume split.
- **Key deliverables:** Payments API; no-card-data customer-admin flow; Subscriptions API; Exigo event feed; PCI scope reassessment.
- **Dependencies:** Fluid API capabilities for hosted capture / subscription events / 2-way sync **[VERIFY — §13]**; Phase 1.
...
### Phase 3 — Profile & Family/Friends extraction
...
- **Workstreams:**
  ...
  - Unified **Signup/Upgrade** application across customer and affiliate, built on Phases 2–3 services.
...
### Phase 5 — Analytics & AI "offense"
- **Goal:** Data drives decisions; first customer-facing AI features ship.
- **Workstreams:**
  - Event-fed **data warehouse**; business-health dashboards (§9) including billing-integrity and field-health metrics.
...
- **Dependencies:** Services emitting events (Phases 2–3); leadership agreement on KPIs.
- **Exit criteria:** Leadership reviews a single source of business-health truth; at least one AI feature is in production.

> **Parallelization note:** Phases 0–1 are strictly sequential and gating. Phases 2–3 can overlap once foundations exist; Phase 4 trails 2–3; Phase 5 can start its data work during Phase 3.
...
|---|----------|-----------------|
| 13.1 | Payment/billing behavior at the tokenization seam | **Resolved for this draft** — framing reduced to *"tokenization happens in multiple places with no single authority."* (Double-charge narrative removed.) |
| 13.2 | Profile source of truth | **Resolved — Exigo.** Profile *service* is a facade; Exigo remains the record. |
| 13.3 | Card data on Limbic Arc sites / PCI scope | **Target (unverified):** Limbic Arc sites should collect **no** credit-card info and be **out of PCI scope.** **[VERIFY current state.]** |
| 13.4 | Merchant account(s) & current PCI SAQ level | **Yet to be verified.** |
| 13.5 | Replicated-site usage | **Clarified:** low usage = **low functionality, but used by *all* users.** On the critical path; cannot be retired as low-priority. |
| 13.6 | Fluid API surface (hosted capture, subscription events, 2-way sync) | **To be confirmed.** Gates Phase 2 design. |
| 13.7 | Delivery capability | **Confirmed: no in-house technology team currently.** Phase 0 must establish a team and/or integration partner. |
| 13.8 | Web App scope boundary (SSO-only?) | **To be confirmed.** |
...
*End of draft v0.2. Next iteration should fold in answers to the remaining [VERIFY] items (§13.3 current PCI state, §13.4 merchant/SAQ, §13.6 Fluid API capabilities, §13.8 Web App scope), and right-size §12 once the Phase 0 team/partner model is chosen.*
